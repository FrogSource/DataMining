...
class ApiHandler(object):
    ...
    def get_tweets_by_keyword(self):
        ...
        newresults = results = self.api.GetSearch(term=self.query_word,
        ...
        while True:
            count = max(const.MAX_COUNT_KEYWORD - len(results), 100)

            for tweet in newresults:
                if tweet.id < max_id:
                    max_id = tweet.id

            try:
                newresults = self.api.GetSearch(term=self.query_word,
                                     count=count,
                                     include_entities=const.INCLUDE_ENTITIES,
                                     max_id=max_id)

                results += newresults
                results = filter(lambda x: x.retweeted_status is None, results)

                logging.info("Retrieved %i new tweets. Total: %i ",  len(newresults), len(results))

                if len(newresults) == 0 or (len(results) >= const.MAX_COUNT_KEYWORD):
                    break
